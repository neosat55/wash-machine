...
export class BoxRepository {
  ...
  getBoxes() {
    return this.client.selectFrom('boxes')
      .selectAll()
      .execute();
  }

  addNewBox(box: Omit<BoxModel, 'id' | 'masters'>) {
    return this.client.insertInto('boxes')
      .values(box)
      .returning('id')
      .executeTakeFirst();
  }
  ...
  getBoxStatus(id: number) {
    return this.client.selectFrom('boxes_queue').where('box_num', '=', id).select('status').executeTakeFirst();
  }

  getBoxMasters(id: number) {
    return this.client.selectFrom('boxes_masters').where('box_id', '=', id)
      .selectAll()
      .execute();
  }

  getBox(id: number) {
    return this.client.selectFrom('boxes').selectAll().where('id', '=', id).executeTakeFirst();
  }

  async increaseQueue(boxId: number, trx: Transaction<DB>) {
    await this.client.updateTable('boxes_queue')
      .set(eb => ({
  ...
  addBoxMaster(box_id: number, master_id: number) {
    return this.client.insertInto('boxes_masters')
